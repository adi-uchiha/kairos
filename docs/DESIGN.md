...
_Hover state:_ Slightly dimmed (`opacity: 0.85` or light scale overlays).

### **`button-secondary`**

Transparent background, 1px Hairline border (`var(--border)`), `var(--text-primary)` text, `0px` radius.
_Hover state:_ Surface dark fill (`var(--surface-hover)`).
...
_Active state:_ Orange border, Orange wash fill.
...
_Kairos Design is about eliminating the noise so the architecture can speak for itself._
